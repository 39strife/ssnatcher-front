...
import { useModalContext, Modal } from "./ModalContext";
...
export const ProtectedRoute = ({ permissions }) => {
  const { isAuthenticated, user } = useAuth();
  const router = useRouter();
  useEffect(() => {
    if (!isAuthenticated) {
      router.push("/unauthorized?reason=login");
    }
    if (permissions && permissions > user.role) {
      router.push("/unauthorized?reason=permissions");
    }
  }, []);
  return <></>;
};
...
export const AuthModals = () => {
  const { login, register } = useModalContext();
